...
// 但此前未回填到 sqlite 模板。deck_view 的 FK 需要 deck 存在，这里只补齐
...
export const brand = table(
  'brand',
  {
    ...
    palette: text('palette', { mode: 'json' }).$type<Record<string, string>>(),
...
export const deck = table(
  'deck',
  {
    ...
    expiresAt: integer('expires_at', { mode: 'timestamp_ms' }),
    noIndex: integer('no_index', { mode: 'boolean' }).notNull().default(true),
